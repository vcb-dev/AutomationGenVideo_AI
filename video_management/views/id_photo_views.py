...
def merge_outfit(request):
    ...
    model_name = str(
        getattr(settings, 'GEMINI_MODEL', None) or os.getenv('GEMINI_MODEL') or DEFAULT_GEMINI_IMAGE_MODEL or ''
    ).strip()
    if not model_name:
        return Response({
            'success': False,
            'error_message': 'GEMINI_MODEL chưa được set trong .env trên AI Service.',
        }, status=500)
