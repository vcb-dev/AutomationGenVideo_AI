...
def scrape_instagram_reels(username: str, count: int = 100) -> list:
...
def ingest_instagram_reels(items: list) -> dict:
    ...
    for item in sorted_items:
        ...
        if isinstance(caption_obj, dict):
            ...
            # caption.hashtags đã được TikHub extract sẵn (kèm #)
        ...
        # thumbnail_url có thể null, fallback sang image_versions.items[0]
        ...
        defaults = {
            ...
            'views_count': item.get('play_count') or item.get('ig_play_count') or 0,
        ...
        # Upload thumbnail lên Drive (non-blocking) — CDN URL từ TikHub expire ~24h
        # New reels: luôn upload. Updated reels: chỉ upload nếu chưa có Drive URL.
