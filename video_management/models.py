...
class ReportSettings(BaseModel):
    """
    Cấu hình khung giờ cho phép báo cáo checklist.
    
    Manager có thể set khung giờ khác nhau cho từng ngày trong tuần.
    Mỗi organization chỉ có 1 settings record (singleton pattern).
    """
    # Khung giờ cho từng ngày trong tuần (JSON format)
    # Ví dụ: {"monday": {"start": "07:00", "end": "09:00", "enabled": true}, ...}
    schedule = models.JSONField(
        default=dict,
        help_text="Khung giờ cho phép báo cáo theo ngày trong tuần"
    )
    
    # Có bật giới hạn 1 lần/ngày không
    ...
    timezone = models.CharField(
        ...
        help_text="Timezone để check thời gian"
    )
    ...
    updated_by = models.CharField(
        max_length=255,
        blank=True,
        help_text="Email của manager cập nhật settings lần cuối"
    )
    ...
    def __str__(self) -> str:
        return f"Report Settings (Updated: {self.updated_at})"
    
    @classmethod
    def get_settings(cls):
        """
        Lấy settings hiện tại. Nếu chưa có thì tạo default.
        Singleton pattern - chỉ có 1 record duy nhất.
        """
        settings, created = cls.objects.get_or_create(
            pk=1,  # Always use ID=1 for singleton
            defaults={
                'schedule': cls.get_default_schedule(),
    ...
    def get_default_schedule():
        """
        Lịch mặc định: T2-T6: 7:00-9:00, T7-CN: 8:00-10:00
        """
        return {
            "monday": {"start": "07:00", "end": "09:00", "enabled": True},
            "tuesday": {"start": "07:00", "end": "09:00", "enabled": True},
            "wednesday": {"start": "07:00", "end": "09:00", "enabled": True},
            "thursday": {"start": "07:00", "end": "09:00", "enabled": True},
            "friday": {"start": "07:00", "end": "09:00", "enabled": True},
            "saturday": {"start": "08:00", "end": "10:00", "enabled": True},
            "sunday": {"start": "08:00", "end": "10:00", "enabled": True},
        }
    
    def is_report_allowed(self, check_datetime=None):
        """
        Kiểm tra xem có được phép báo cáo tại thời điểm này không.
        
        Args:
            check_datetime: Datetime object để check (mặc định: now)
        ...
        # Lấy tên ngày (monday, tuesday, ...)
        day_name = check_datetime.strftime('%A').lower()
        
        # Lấy schedule của ngày này
        day_schedule = self.schedule.get(day_name)
        if not day_schedule:
            return False, f"Không có cấu hình cho {day_name}"
        
        if not day_schedule.get('enabled', False):
            return False, f"Báo cáo bị tắt vào {day_name}"
        ...
        try:
            ...
            if not (start_time <= current_time <= end_time):
                return False, f"Chỉ được báo cáo từ {start_time_str} đến {end_time_str}"
            
            return True, "OK"
            
        except (ValueError, KeyError) as e:
            return False, f"Cấu hình không hợp lệ: {str(e)}"
